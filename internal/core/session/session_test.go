...
func TestSessionCompletesAfterDuration(t *testing.T) {
	...
	m.StartBreak(now, &scheduler.Event{Reasons: []scheduler.ReminderType{scheduler.ReminderEye}, BreakSec: 20}, true)
	if !m.IsActive() {
...
func TestSkipRespectsCanSkip(t *testing.T) {
	...
	m.StartBreak(now, &scheduler.Event{Reasons: []scheduler.ReminderType{scheduler.ReminderStand}, BreakSec: 60}, false)
	if err := m.Skip(); err == nil {
		t.Fatalf("expected error when skip is disabled")
	}

	m.StartBreak(now, &scheduler.Event{Reasons: []scheduler.ReminderType{scheduler.ReminderStand}, BreakSec: 60}, true)
	if err := m.Skip(); err != nil {
